    ...
    <div className="w-full flex flex-col items-center py-8">
      ...
      <div className="flex flex-wrap justify-center gap-4 w-full max-w-md px-4">
        {users.map((user) => {
          ...
          const avatarUrl = USER_AVATARS[user];

          return (
            <button
              key={user}
              onClick={() => onChange(user)}
              className={`w-[calc(50%-0.5rem)] flex flex-col items-center justify-center py-8 px-4 gap-4 bg-surface-highest transition-all ${
                isActive ? 'ring-2 ring-primary ring-inset' : 'hover:bg-surface'
              }`}
            >
              ...
              <span className={`font-display text-sm font-bold tracking-widest uppercase mt-2 ${
